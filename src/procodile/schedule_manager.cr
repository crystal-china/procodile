  ...
  class ScheduleManager
    ...
    def reload_schedules : Nil
      ...
      scheduled_jobs.keys.each do |name|
        ...
        @supervisor.issue_tracker.resolve(:invalid_schedule, name)
        @supervisor.issue_tracker.resolve(:scheduled_run_failed, name)
    ...
    def finish_scheduled_instance(instance : Instance) : Nil
      ...
      if stopped_by_user || instance.process.last_exit_status == 0
        @supervisor.issue_tracker.resolve(:scheduled_run_failed, process_name)
      else
...
or `#{suggested_restart_command}`."
    ...
    private def clear_scheduled_skip_state(name : String) : Nil
      ...
      @supervisor.issue_tracker.resolve(:scheduled_run_skipped_repeatedly, name)
    end
